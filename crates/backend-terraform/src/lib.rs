...
mod tests {
    ...
    fn media_jellyfin_bootstrap_adds_virtual_folders_via_query_params() {
        ...
        assert!(script.contains("def transcode_probe(token: str, user_id: str) -> bool:"));
        assert!(script.contains("def probe_transcode_candidate(token: str, item_id: str, media_source_id: str) -> bool:"));
        assert!(script.contains("def set_encoding_mode("));
        assert!(script.contains("probe_session_id = str(uuid.uuid4())"));
        assert!(script.contains("Fields=Path,MediaSources,MediaStreams"));
        assert!(script.contains("VideoCodec\": \"av1,h264,vp9\""));
        assert!(script.contains("VideoBitrate\": \"2147099647\""));
        assert!(script.contains("AudioBitrate\": \"384000\""));
        assert!(script.contains("/hls1/main/-1.mp4"));
        assert!(script.contains("actualSegmentLengthTicks=0"));
        assert!(script.contains("str(stream.get(\"Codec\") or \"\").strip().lower() == \"hevc\""));
        assert!(script.contains("has_dovi_or_hdr"));
        assert!(script.contains("stream.get(\"VideoDoViTitle\")"));
        assert!(script.contains("stream.get(\"DvProfile\")"));
        assert!(script.contains("source_hints"));
        assert!(script.contains("\"dv.\","));
        assert!(script.contains("prioritized = [row for row in candidates if row[\"has_dovi_or_hdr\"] and row[\"has_hevc\"]]"));
        assert!(script.contains("if len(probes) >= 8:"));
        assert!(script.contains("fallback=software"));
        assert!(script.contains("fallback=safe-hw"));
        assert!(script.contains("JELLYFIN_SOFTWARE_TRANSCODE_MARKER"));
        assert!(script.contains("transcode_probe(token, admin_user_id)"));
        assert!(script.contains("JELLYFIN_HWACCEL_TYPE\", \"none\""));
        assert!(script.contains("JELLYFIN_HWACCEL_ENABLE_ENCODING\", \"false\""));
        assert!(script.contains("JELLYFIN_HWACCEL_PREFER_NATIVE_DECODER\", \"false\""));
    ...
    fn media_caddy_fixture_uses_service_port_mode_with_stremio_jellyfin_proxy() {
        ...
        assert!(caddy.contains("handle /items/*"));
        assert!(caddy.contains("consistent direct-play/transcode fallback decisions"));
        ...
        assert!(caddy.contains("header_up X-MediaBrowser-Token {$JELLYFIN_AUTO_AUTH_TOKEN}"));
    ...
    fn media_env_fixture_derives_public_hosts_from_resource_identity() {
        ...
        assert!(env.contains("JELLYFIN_HWACCEL_ENABLE_TONEMAPPING=false"));
        assert!(env.contains("JELLYFIN_HWACCEL_ENABLE_VPP_TONEMAPPING=false"));
    ...
    fn streaming_validation_fixture_checks_tizen_catalogs() {
        ...
        assert!(script.contains("validation failed: Jellio stream probe returned HTTP"));
        assert!(script.contains("Jellio stream payload returned no streams"));
        assert!(script.contains("stream_meta_url ="));
        assert!(script.contains("urllib.parse.quote(stream_id, safe='')"));
        assert!(script.contains("if err.code in (301, 302, 303, 307, 308):"));
